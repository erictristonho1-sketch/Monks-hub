...
local mobileSupport = LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("MobileSupport", 10)
local holdConnection = nil
local checkTime = 0
local requiredHoldTime = 1.5 -- Tempo em segundos para segurar

if mobileSupport then
    local frame = mobileSupport:WaitForChild("Frame", 10)
    if frame then
        local flowBtn = frame:FindFirstChild("Flow") or frame:FindFirstChild("FlowButton")
            ...
            flowBtn.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    holdConnection = RunService.Heartbeat:Connect(function()
                        if tick() - checkTime >= requiredHoldTime then
                            -- Tempo atingido, alterna a GUI e para de checar
                            Drag.Visible = not Drag.Visible
...
RadarFrame.Name = "AllyRadar"
RadarFrame.Size = UDim2.new(1,0,1,0)
RadarFrame.Position = UDim2.new(0,0,0,0)
RadarFrame.BackgroundTransparency = 1
RadarFrame.Parent = RRR

local Holder = Instance.new("Folder")
Holder.Name = "Icons"
Holder.Parent = RadarFrame

local BORDER = 40 -- Distância da borda da tela

--====================================================
-- CRIAR ICONE DO JOGADOR
--====================================================
...
local function createIcon(plr, pos, dist)
	...
	img.Image = "rbxassetid://10618585472" -- Icone de seta/setinha
	img.Parent = frame

	-- Rotação para apontar pro centro
	...
	if plr.Team then
		stroke.Color = plr.Team.TeamColor.Color
	else
